...
impl RootSpanBuilder for DefaultRootSpanBuilder {
    fn on_request_start(request: &ServiceRequest) -> Span {
        root_span!(level = tracing::Level::INFO, request)
    }
...
#[derive(Debug, Clone, Copy)]
pub struct MinimalRootSpanBuilder;

impl RootSpanBuilder for MinimalRootSpanBuilder {
    fn on_request_start(_request: &ServiceRequest) -> Span {
